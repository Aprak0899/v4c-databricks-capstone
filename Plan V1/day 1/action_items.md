...
- [ ] Create branch **feature/data-profiling** and do all Day 1 work on it (no direct commits to main/dev).
- [ ] After Day 1 is tested and error-free: merge **feature/data-profiling** into **dev**.
...
- [ ] Complete **Requirement Documentation and Assumption** (e.g. docs/01_discovery_and_assumptions.md or equivalent); include scope, limitations, assumptions.
- [ ] Complete **Dataset Profiling** (run profiling notebook on raw files; record row counts, columns, nulls, sample); document findings (e.g. profiling_findings.md or in discovery doc).

---

## Sign-off before merge to dev

- [ ] All Day 1 deliverables present: repo structure, Git integration, GitHub Actions, databricks.yml, Requirement Doc, Dataset Profiling, Data Chunking notebook + Job, multiple run scenarios.
- [ ] Code on **feature/data-profiling** is tested and error-free.
- [ ] Merge **feature/data-profiling** into **dev**.
...
| Deliverable | Done |
|-------------|------|
| PRIVATE GitHub repo vstone-databricks-pipeline with proper structure | |
...
| Git: feature/data-profiling merged to dev after testing | |
| Notebook code handles multiple run scenarios | |
