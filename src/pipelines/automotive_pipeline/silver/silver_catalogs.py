"""
Silver: Typed catalogs with unit stripping (Day 4).
- catalogs_silver: standardized metrics, UTC dates; only rows with valid unit format.
- catalogs_quarantine: rows that fail unit validation (for audit).
"""
from pyspark import pipelines as dp
import pandas as pd
from pyspark.sql.functions import col, pandas_udf, to_utc_timestamp, regexp_extract
from pyspark.sql.types import DoubleType

# Pattern for valid clearance (must end with мм, см, or м)
VALID_UNIT_PATTERN = ".*(мм|см|м)$"
...
def clean_units_udf(unit_series: pd.Series) -> pd.Series:
    ...
    """
    extracted = unit_series.str.replace(',', '.', regex=False).str.extract(r'(\d+\.?\d*)')
    return extracted.iloc[:, 0].astype(float)


# ---------------------------------------------------------------------
# 2. SILVER TABLE (Standardized & Typed) — expectation drops invalid rows
# ---------------------------------------------------------------------
@dp.table(
...
def catalogs_silver():
    ...
    return (
        bronze_df.select(
            "model",
            "generation",
            col("marka").alias("brand"),
            col("complectation").alias("trim_level"),
            col("clearance"),  # Keep original so expectation can validate it
...
# ---------------------------------------------------------------------
# 3. QUARANTINE TABLE (Rescue: invalid unit format for manual audit)
# ---------------------------------------------------------------------
@dp.table(
...
def catalogs_quarantine():
    bronze_df = dp.read("catalogs_raw")
    return bronze_df.filter(~col("clearance").rlike(VALID_UNIT_PATTERN))